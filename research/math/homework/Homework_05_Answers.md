...
对于 $\theta \ne 0$，由于 $X^n = \begin{pmatrix} (i\theta)^n & (i\theta)^{n-1} z \\ 0 & 0 \end{pmatrix}$ （$n \ge 1$），可以算出：
...
**不是单射**：取 $\theta = 2\pi$，任意 $z \in \mathbb{C}$，都有 $\exp \begin{pmatrix} 2\pi i & z \\ 0 & 0 \end{pmatrix} = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix} = I$，因此 $\exp$ 不是单射。
**是满射**：给定 $G$ 中任意元素 $\begin{pmatrix} a & b \\ 0 & 1 \end{pmatrix}$。若 $a = 1$，取 $\theta = 0, z = b$。若 $a \ne 1$，可设 $a = e^{i\theta}$ ($\theta \in (0, 2\pi)$)，由于此时 $\frac{e^{i\theta}-1}{i\theta} \ne 0$，我们只需取 $z = b \frac{i\theta}{e^{i\theta}-1}$ 即可，所以 $\exp$ 是满射。

(c) $\tilde{G}$ 的李代数由 $Y = \begin{pmatrix} i\theta & 0 & z \\ 0 & 0 & \theta \\ 0 & 0 & 0 \end{pmatrix}$ 组成。
计算其指数映射（通过泰勒展开）：
$\exp(Y) = I + Y + \frac{Y^2}{2} + \dots = \begin{pmatrix} e^{i\theta} & 0 & \frac{e^{i\theta} - 1}{i\theta} z \\ 0 & 1 & \theta \\ 0 & 0 & 1 \end{pmatrix}$ （当 $\theta \ne 0$ 时），当 $\theta = 0$ 时为 $\begin{pmatrix} 1 & 0 & z \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}$。
**不是满射**：要得到 $\tilde{G}$ 中的形如 $\begin{pmatrix} 1 & 0 & b \\ 0 & 1 & 2\pi \\ 0 & 0 & 1 \end{pmatrix}$ ($b \ne 0$) 的元素，根据第三列第二行的项，必须有 $\theta = 2\pi$。但一旦 $\theta = 2\pi$，第一行第三列的项 $\frac{e^{2\pi i} - 1}{2\pi i} z$ 必然为 $0$，不可能等于非零的 $b$。所以不是满射。
**不是单射**：对于不同的 $z_1, z_2$，取 $\theta = 2\pi$，由于 $\frac{e^{2\pi i}-1}{2\pi i} = 0$，指数映射的值均为 $\begin{pmatrix} 1 & 0 & 0 \\ 0 & 1 & 2\pi \\ 0 & 0 & 1 \end{pmatrix}$，所以不是单射。
